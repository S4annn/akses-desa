...
export function ChatbotPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      role: 'assistant',
      content: 'Halo! Saya AI Desa, siap membantu menjawab pertanyaan tentang layanan, pengaduan, agenda, atau bantuan sosial. Silakan tanyakan apa saja.',
    },
  ]);
  ...
  async function handleSend(text?: string) {
    ...
    try {
      ...
      setMessages((m) => [...m, { id: Date.now() + 1, role: 'assistant', content: 'Maaf, saya sedang tidak bisa menjawab. Coba lagi nanti.' }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div>
      <PageHeader eyebrow="AI Desa" title="Tanya AI Layanan Desa" description="Asisten cerdas yang membantu warga menemukan informasi cepat." />
          ...
          <div className="card flex h-[70vh] min-h-[520px] flex-col p-0 lg:col-span-2">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4">
              {messages.map((m) => (
                ...
                >
                  <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full ${m.role === 'user' ? 'bg-brand-700 text-white' : 'bg-slate-100 text-slate-700'}`}>
                    {m.role === 'user' ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
                  </span>
                  <div className={`max-w-[78%] whitespace-pre-line rounded-2xl px-3.5 py-2 text-sm ${m.role === 'user' ? 'bg-brand-700 text-white' : 'bg-slate-100 text-slate-800'}`}>
                    {m.content}
                  </div>
                </motion.div>
              ))}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <button disabled={loading} className="btn-primary"><Send className="h-4 w-4" /></button>
              </div>
          ...
          <div className="space-y-4">
            <div className="card p-5">
              ...
              <div className="mt-3 flex flex-col gap-2">
                {quick.map((q) => (
                  <button key={q} onClick={() => handleSend(q)} className="rounded-xl border border-slate-200 px-3 py-2 text-left text-sm text-slate-700 hover:border-brand-300 hover:bg-brand-50/40">
                    {q}
                  </button>
                ))}
              </div>
            ...
            <div className="card p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {Array.from(new Set(knowledgeBase.map((k) => k.category))).map((c) => (
                  <span key={c} className="chip border border-slate-200 bg-slate-50 text-slate-700">{c}</span>
                ))}
              </div>
            </div>
          </div>
